                  ...
                  { matrix =
                    ...
                    , name =
                        Prelude.List.map
                          MatrixEntry.Type
                          Text
                          (\(entry : MatrixEntry.Type) -> entry.name)
                          entries
                    }
                  }
...
let make_condition =
      ...
      ->  Prelude.Text.concatMapSep
            ...
            (     \(condition : Condition)
              ->  merge
                    { event_name = \(x : Text) -> "github.event_name == '${x}'"
                    , ref = \(x : Text) -> "github.ref == '${x}'"
                    , repository = \(x : Text) -> "github.repository == '${x}'"
                    , success =
                        \(x : Bool) -> "${if x then "" else "!"}success()"
                    }
                    condition
            )
            conditions
...
let make_job =
      ...
      ->      job
          //  { strategy = make_strategy job.strategy
              ...
              , steps =
                  Prelude.List.map Step.Type StepOutput make_step job.steps
              }
...
let make_ci = \(ci : CI.Type) -> ci // { jobs = make_jobs ci.jobs }
